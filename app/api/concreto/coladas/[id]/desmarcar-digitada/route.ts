...
import { desmarcarDigitada } from '@/lib/concreto/coladas-workflow';
import { obtenerColada } from '@/lib/concreto/coladas';

// POST /api/concreto/coladas/[id]/desmarcar-digitada — digitada → confirmada.
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const r = await desmarcarDigitada(db, idColada, actor);
    if (!r.ok) {
    ...
    console.error('/api/concreto/coladas/[id]/desmarcar-digitada error:', err);
