...
import { excluirBatch } from '@/lib/concreto/coladas-workflow';
import { obtenerColada } from '@/lib/concreto/coladas';

// POST /api/concreto/coladas/[id]/excluir-batch/[idBatch] — Body: { motivo: string }
export async function POST(
  ...
  if (!motivo || motivo.length > 500) {
    return NextResponse.json({ error: 'Se requiere motivo (1..500 caracteres).' }, { status: 400 });
  }

  try {
    const db = await getAdelanteDb();
    const r = await excluirBatch(db, idColada, idBatchNum, motivo, actor);
    if (!r.ok) {
    ...
    console.error('/api/concreto/coladas/[id]/excluir-batch/[idBatch] error:', err);
