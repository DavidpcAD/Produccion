...
import { desconfirmar } from '@/lib/concreto/coladas-workflow';
import { obtenerColada } from '@/lib/concreto/coladas';

// POST /api/concreto/coladas/[id]/desconfirmar — confirmada → sugerida.
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const r = await desconfirmar(db, idColada, actor);
    if (!r.ok) {
    ...
    console.error('/api/concreto/coladas/[id]/desconfirmar error:', err);
