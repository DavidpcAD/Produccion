...
import { getTipoObra, tipoObraDeAreaCosteo } from '@/lib/partidas/tipos-obra';
import {
  areaCosteoDeObra,
  armarJerarquia,
  catalogoDeObra,
 ...
 *
 * El tipo de obra sale del ÁREA DE COSTEO de la obra (dbo.Obra →
 * pro_obc.tipo_obra_area_costeo), así que el cruce es contra el catálogo correcto:
...
export async function POST(req: NextRequest) {
  ...
  const area = await areaCosteoDeObra(worksNo);
  if (area === null) {
  ...
  const tipoCodigo = await tipoObraDeAreaCosteo(area);
  const tipo = await getTipoObra(tipoCodigo);
  if (!tipo) {
    return NextResponse.json(
      { error: `El área de costeo "${area}" apunta al tipo de obra "${tipoCodigo}", que no existe en pro_obc.tipos_obra.` },
      { status: 500 },
    );
  }
