...
export default function ProveeduriaSolicitudesPage() {
  ...
  const columns = useMemo<ColumnDef<Pedido, any>[]>(() => [
    ...
    { id: "tipo", header: "Tipo", accessorFn: (p) => tipoSolicitudBadge(p.tipoSolicitud).label, meta: { label: "Tipo" }, cell: (c) => { const t = tipoSolicitudBadge(c.row.original.tipoSolicitud); return <Badge tone={t.tone}>{t.label}</Badge>; } },
    {
      ...
      cell: (c) => { const p = c.row.original; return <div><div className="ds-strong ds-body-sm">{destinoCodigo(p)}</div><div className="ds-muted ds-body-sm ds-wrap" style={{ minWidth: 150, maxWidth: 200 }}>{destinoLabel(p)}</div></div>; },
    },
    { id: "comentario", header: "Comentario", accessorFn: (p) => p.notas ?? "", meta: { label: "Comentario" }, cell: (c) => <div className="ds-body-sm ds-muted ds-cell-texto">{c.getValue() || "—"}</div> },
