...
export default function SeguimientoPage() {
  ...
  const columns = useMemo<ColumnDef<Fila, any>[]>(() => [
    ...
    { id: "articulo", header: "Artículo", accessorFn: (f) => f.articulo, meta: { label: "Artículo" }, cell: (c) => { const f = c.row.original; return <div className="row gap-2" style={{ alignItems: "center" }}><div className="ds-cell-nombre">{f.articulo}</div>{f.deBaja > 0 && <Badge tone="gray" title={`Se archivó la solicitud: ${num.format(f.deBaja)} ${f.unidad} nunca se ordenaron y ya no se van a comprar.`}>Ya no se compra</Badge>}</div>; } },
