...
import { almacenesDeRecepcion, ordenEsParcial, ordenesDelAlcance, type AlcanceRecepcion } from "@/lib/compras/helpers";
      ...
      <main className="page">
        <div className="page__head">
          <div className="page__title">
            ...
            <p className="ds-muted">Registrá la factura cuando el material llega a bodega. Soporta entregas parciales. Tocá un panel para filtrar.</p>
          </div>
        </div>
        ...
        {/* Vacío por el SELECTOR (no porque no haya nada): la salida es quitar el
            filtro, no mandar a la persona a buscar en otra pestaña. */}
        {filtro === "porRecibir" && porRecibir.length === 0 ? (
          <Card><div className="empty" style={{ lineHeight: 1.6 }}>No hay órdenes pendientes de recibir.<br />{alcance !== "todas"
            ? <button className="link-btn" onClick={() => setAlcance("todas")}>Estás viendo solo {alcance === "fabrica" ? "las de tu fábrica" : "las de tus solicitudes"} — ver todas</button>
            : <span className="ds-muted ds-body-sm">Cuando proveeduría lance una orden, aparece acá.</span>}</div></Card>
        ) : (
