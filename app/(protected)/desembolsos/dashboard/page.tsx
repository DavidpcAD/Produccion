    ...
    <main className="page mx-auto w-full max-w-6xl px-4 py-6">
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
        <div>
          ...
          <h2 className="mt-1 text-6xl font-black leading-none tabular-nums text-black">
      ...
      <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-4">
        ...
        />
        <div className="flex items-center gap-1 rounded-ds border-2 border-transparent bg-white p-1 shadow-ds-01">
        ...
        {opciones.map((o) => (
          <button
            ...
            className={`px-3 py-1.5 text-[11px] uppercase tracking-wider transition ${
              valor === o.valor ? 'bg-black text-white' : 'bg-white text-ds-gray-500 hover:bg-ds-gray-100'
            }`}
          >
            {o.label}
          </button>
      ...
      <div className="flex items-end gap-4 border-b border-ds-gray-200 pb-1" style={{ height: 180 }}>
        {serie.map((s) => {
          ...
          return (
            ...
            >
              <span className="text-[11px] tabular-nums text-black">{fmtCorto(s.Total)}</span>
  ...
  return (
    <tr className="cursor-pointer border-b border-ds-gray-100 last:border-0 hover:bg-ds-gray-100/60" onClick={onClick}>
      <td className="px-4 py-3">
        ...
        <div className="font-medium text-black">{caso.Cliente}</div>
      </td>
      <td className="px-4 py-3">
        <div className="text-xs text-black">{caso.NombreModelo ?? '—'}</div>
      ...
      <td className="px-4 py-3 text-right">
        <div className="text-sm font-semibold tabular-nums text-black">{fmtCorto(caso.PrecioVenta_CRC ?? 0)}</div>
      </td>
      <td className="px-4 py-3 text-right">
        <div className="text-sm font-semibold tabular-nums text-black">{fmtCorto(pendiente)}</div>
        ...
        {esReservado ? (
          ...
          <div className="min-w-[100px]">
            <div className="mb-1 flex items-center justify-between text-[10px]">
              <span className="font-semibold tabular-nums text-black">{Number(caso.PorcentajeAvance ?? 0).toFixed(0)}%</span>
      ...
      <td className="px-4 py-3">
        {caso.ProximoMonto_CRC ? (
          <>
            <div className="text-xs font-semibold tabular-nums text-black">{fmtCorto(proximoMonto)}</div>
            <div className="text-[10px] text-ds-gray-400">
