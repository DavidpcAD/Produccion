...
export async function calcularHistorico(
    ...
    .query<HistoricoObra>(
      hayFoto
        ? `SELECT aso.obra_codigo AS codigo, e.tipo_casa, aso.sprint_actual
           FROM pro_obc.avance_semanal_obra aso
           JOIN pro_obc.obra_estado e
    ...
    tieneBaseLinea && nextLbSem != null
      ? await db
          ...
          .query<{ obra_codigo: string; sub_partida_id: number; pct: number }>(
            'SELECT obra_codigo, sub_partida_id, pct_completado AS pct FROM pro_obc.avance_base_semanal WHERE semana_operativa_id = @next',
          )
      : tieneBaseLinea
        ? await db
            .request()
            .query<{ obra_codigo: string; sub_partida_id: number; pct: number }>(
              'SELECT obra_codigo, sub_partida_id, pct_completado AS pct FROM pro_obc.avance_sub_partidas',
            )
        : cerrada
          ? await db
              .request()
  ...
  const priorQ = tieneBaseLinea
    ? await db
        ...
        .input('sem', sql.BigInt, semanaId)
        .query<{ obra_codigo: string; sub_partida_id: number; pct: number }>(
          'SELECT obra_codigo, sub_partida_id, pct_completado AS pct FROM pro_obc.avance_base_semanal WHERE semana_operativa_id = @sem',
        )
    : await db
        .request()
        .input('sem', sql.BigInt, semanaId)
